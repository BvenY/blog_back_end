...
router.post('/', token, (req, res) => {
    ...
    /*对必要数据进行判空*/
    if (!("userID" in requestData && "msgType" in requestData && "message" in requestData)) {
    ...
    // 定义SQL语句
    const sqlStr = `INSERT INTO usermsg(userID,msgType,message) VALUES(?,?,?)`;
    //定义要插入的数据
    let sqlData = [userID, msgType, message];
